...
>
<p>
The first of such tools developed was a gender parser and tagger groomed specifically for the Wedding Announcements corpus. While moderate success was achieved through the use of gazetteers and traditional lexicons such as Wordnet (Wordnet) (a success rate of 85%) was achieved, there was still much room for improvement. Exploiting the shallow diversity of names represented in the Times and the uniform sentence structure of the articles(Named Entity Recognition without Gazetteers:), the gender classifier was trained on a small corpus of 100 randomly selected names manually chosen from the wedding announcements. Feeding this corpus back into our Bayesian gender classifier increased accuracy across the corpus to 92%, hinting at the potential benefits of using locally trained classifiers for semi-structured text-based corpuses (Acquisition of Semantic Patterns for Information Extraction from Corpora:).
</p>
</TextBlock>
...
>
<p>
As the study progressed, one attribute of the data that became of increasing interest was the Universities and Institutions of Higher Education that are tied to those featured in the Wedding Announcements. To retrieve this information a parsing tool was developed which specifically targeted the names of American Colleges and Universities (Named Entity Recognition from Diverse Text Type). Our tool used the list of Barron’s top ranked universities as a gazetteer and was complemented by NLTK (CITATION NEEDED) as a tool for extracting named entities from text-based corpuses using a prescribed seed database (or gazetteer) (Learning Dictionaries for Information Extraction by Multi-Level Bootstrapping). This tool, aided greatly by the fact that college and university names are generally capitalized in a corpus and are thereby recognized quite easily by generic Named Entity recognition tools and algorithms, was able to generate accurate classifications and detections of American Colleges and Universities(Design Challenges and Misconceptions in NER).
</p>
<p>
Lastly, using design principles very similar to those used for the construction of the previous lexer, a system for detecting and tagging individuals careers and occupations was developed. Occupations, unlike most Named Entities, does not large corpus of tagged items residing in a publicly available gazetteer or dictionary (Named Entiry Recognition without Gazetteers). Hence, in order to construct a reliable system for detecting occupations and careers from the database of collected articles, a novel and reliable system needed to be constructed (Named Entity Recognition in Tweets: An Experimental Study).
</p>
<p>
To accurately recognize careers and occupations as Named Entities, the lexer exploited the uniform sentence structure and verbiage of the Wedding Announcements via the use of semi-structured regular expression statements (Learning Information Extraction Rules for Semi-structured and Free text). Beginning with a regular expression constructed by hand after manually examining select articles from the corpus the classifier began, the classifier applied the regular expressions to the corpus and made fine tweaks to their specificity after cross referencing results (Learning Dictionaries for Information Extraction by Multi-Level Bootstrapping).
</p>
<p>
Our starting regular expression was “(is)? (a|an)(?P_career_\s\S+){1,3} (with|at)” and our parser added and dropped the prefix and suffix quantifiers if the result matched was similar to other careers or occupations matched thus far(Acquition of Semeantic Patterns for  Information Extraction from Corpora). Unlike previous lexers, this lexer scanned the entire corpus and text of the article, using as much information as possible to strengthen the classifier and the judgments which it made (Named Entity Recognition: A maximum Entrop Approach Using Global Infromation). This collected data was then used to supplement the article database and construct standalone visualizations.
</p>
</TextBlock>
...
<p>
Upon prototyping, it became readily apparent that organizing flow’s programming model in a functional manner would result in immense gains in terms of both developer productivity and rendering performance. Each component, or “element”, in flow models a high-level user-interface component in a completely stateless and declarative manner (ASYNC FUNC REAC PROGRAMMING FOR GUI). Because of the property of each of these elements maintaining their own state, the composition workflow supported by flow is very declarative and simple to debug. Using principles from “Functional Reactive Programming, ” each component only listens to “events” from high-level elements and can only mutate is state in response to such events (A survey on Reactive Programming). This principle creates a hierarchical data-flow that naturally cascades down from parent elements to their children. In addition, since each element maintains only it’s own state, and renders interface elements in a purely functional manner, this method of render interface elements is very fast (ConstrintJS: Programing….). Only those elements whose state is affected as the result of interactions with the document are re-rendered, greatly cutting down on “re-flows” or other very slow and costly operations that have plagued web development for the last century (Adaptive Functional Programming).
</p>
<p>
...
>
<p>
By combining the static abstractions of React with the extensibility of generic data visualization frameworks (such as D3) (D3), flow attempts to strike a balance between declarative, but restrictive, visualization environments and those which are sacrifice extensibility and flexibility at the cost of programming power (Protivis).
</p>
</TextBlock>
...
>
<p>
Building off of the programming methodology for visualizations discussed in the last section, each of the visualization used this workflow was constructed using Mike Bostock’s D3 library (D3 CITATION). As many of the visualizations shared a common theme and code structure, they were written in a generic and reusable manner adhering to  a single interface for insertion into the paper (http://bost.ocks.org/mike/chart/).
</p>
</TextBlock>
